...
if (document.querySelector('.dashboard')) {
  ...
  // ── PORTAL: pindahkan semua .month-list langsung ke <body>
  // agar tidak terpotong oleh overflow:hidden pada .box-inner
  document.querySelectorAll('.month-dropdown').forEach(dropdown => {
    const list = dropdown.querySelector('.month-list');
    if (!list) return;
    // Tandai list agar tahu milik dropdown mana
    list._ownerDropdown = dropdown;
    dropdown._portalList = list;
    document.body.appendChild(list);   // pindah ke body
  });

  // ── Hitung & terapkan posisi fixed berdasarkan posisi selector
  function positionList(dropdown) {
    const selector = dropdown.querySelector('.month-selector');
    const list     = dropdown._portalList;
    if (!selector || !list) return;

    const rect      = selector.getBoundingClientRect();
    const listW     = list.offsetWidth  || 140;
    const listH     = list.scrollHeight || 220;
    const maxH      = Math.min(220, listH);

    let top  = rect.bottom + 6;
    let left = rect.right  - listW;

    // Jangan keluar kanan layar
    if (left + listW > window.innerWidth - 8) left = window.innerWidth - listW - 8;
    // Jangan keluar kiri layar
    if (left < 8) left = 8;
    // Jika tidak muat di bawah → tampilkan di atas selector
  ...
  function openDropdown(dropdown) {
    ...
    document.querySelectorAll('.month-dropdown.active').forEach(d => {
  ...
  // ── Pasang event pada setiap dropdown
  document.querySelectorAll('.month-dropdown').forEach(dropdown => {
    const selector  = dropdown.querySelector('.month-selector');
    ...
    // Event item ada di portal list yang sudah dipindah ke body
    const list  = dropdown._portalList;
    if (!list) return;
  ...
  document.addEventListener('click', () => {
    document.querySelectorAll('.month-dropdown.active').forEach(d => closeDropdown(d));
  });

  // ── Reposisi saat scroll / resize
  window.addEventListener('scroll', () => {
    document.querySelectorAll('.month-dropdown.active').forEach(d => positionList(d));
  }, true);
